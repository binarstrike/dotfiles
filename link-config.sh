...
CONFIG_DIR_HOME=$([ ! -z "$XDG_CONFIG_HOME" ] && echo -n "$XDG_CONFIG_HOME" || echo -n "$HOME/.config")
DOT_DIR_HOME="$HOME"
...
link_config() {
  ...
  ln -svf $config_dir $config_dir_dest
}
...
link_all() {
  # config
  for c in $(ls "$PREFIX_CONFIG_DIR/config"); do
    link_config $c
  done

  # dot file
  for c in $(ls "$PREFIX_CONFIG_DIR/dot"); do
    link_dot $c
  done
}
